...
void GameplayState::HandleCollision(int newPlayerX, int newPlayerY)
{
	...
	if (collidedActor != nullptr && collidedActor->IsActive())
	{
		...
		{
			PowerUp* collidedPowerUp = dynamic_cast<PowerUp*>(collidedActor);
			assert(collidedPowerUp);
			if(!m_player.HasPowerUp())
			{
				m_player.PickupPowerUp(collidedPowerUp);
				collidedPowerUp->Remove();
				m_player.SetPosition(newPlayerX, newPlayerY);
				AudioManager::GetInstance()->PlayKeyPickupSound();
			}
			break;
...
void GameplayState::DrawHUD(const HANDLE& console)
{
	...
	// Checking that key is on player
	/* cout  << " " << Level::WAL
	cout << " power:";
	if(m_player.HasPowerUp())
	{
		m_player.GetPowerUp()->Draw();
	} else
	{
		cout << " ";
	}*/
